...
test.describe('GATE 6 — UAT-04: Teacher TK B Journey (Maria Magdalena, S.Pd.Aud)', () => {
  const evidenceDir = path.resolve('tests/evidence/gate-06/uat-04-teacher-tkb');

  test.beforeAll(() => {
    fs.mkdirSync(evidenceDir, { recursive: true });
  });

  test('UAT-04: Teacher TK B Class Isolation, Attendance Screening, LPPA Draft & State Machine Boundary', async ({ page }) => {
    page.on('pageerror', err => console.log('PAGE_ERR:', err.message));
    page.on('console', msg => console.log('PAGE_LOG:', msg.text()));

    console.log('============================================================');
    console.log('[GATE 6] [UAT-04] STARTING TEACHER TK B JOURNEY (MARIA MAGDALENA)');
    console.log('============================================================');

    await test.step('1. Navigate to Application & Verify Entry State', async () => {
      console.log('[GATE 6] [UAT-04] STEP 01/11: Navigating to Application Entry Point');
    ...
    await test.step('2. Select Persona: Maria Magdalena, S.Pd.Aud (TEACHER - TK B)', async () => {
      console.log('[GATE 6] [UAT-04] STEP 02/11: Selecting Teacher TK B Persona (Maria Magdalena)');
      const teacherBtn = page.locator('button', { hasText: 'Maria Magdalena' });
    ...
    await test.step('3. Verify Authenticated Identity & Resolved Role', async () => {
      console.log('[GATE 6] [UAT-04] STEP 03/11: Verifying Authenticated Context & Role');
      const bodyText = await page.innerText('body');
      expect(bodyText).toContain('Maria Magdalena');
      expect(bodyText).toContain('TEACHER');
      await page.screenshot({ path: path.join(evidenceDir, '02_teacher_tkb_authenticated.png'), fullPage: true });
    });

    await test.step('4. Verify Institutional Context Ribbon', async () => {
      console.log('[GATE 6] [UAT-04] STEP 04/11: Verifying Institutional Ribbon');
      const bodyText = await page.innerText('body');
    ...
    await test.step('5. Inspect Teacher Daily Work & Learning Plans Workspace', async () => {
      console.log('[GATE 6] [UAT-04] STEP 05/11: Inspecting Daily Work & Learning Activities');
      const bodyText = await page.innerText('body');
    ...
    await test.step('6. Navigate to Presensi (Attendance & Health Screening) Workspace', async () => {
      console.log('[GATE 6] [UAT-04] STEP 06/11: Navigating to Attendance Register');
    ...
    await test.step('7. Verify Class Scope Isolation in Attendance Register', async () => {
      console.log('[GATE 6] [UAT-04] STEP 07/11: Verifying Scoped Class Context (Kelompok B)');
      const bodyText = await page.innerText('body');
      expect(bodyText).toContain('Buku Presensi & Skrining Kedatangan Siswa');
      expect(bodyText).toContain('Kelompok B (Matahari Cemerlang)');
    });

    await test.step('8. Navigate to Perkembangan (LPPA Progress Report Drafting)', async () => {
      console.log('[GATE 6] [UAT-04] STEP 08/11: Opening LPPA Development Workspace');
    ...
    await test.step('9. Assert Teacher State Machine Boundary (No Approval / No Publish)', async () => {
      console.log('[GATE 6] [UAT-04] STEP 09/11: Asserting State Machine Boundary (Teacher Forbidden from Approval)');
      const bodyText = await page.innerText('body');
    ...
    await test.step('10. Open Persona Menu to Initiate Sign Out', async () => {
      console.log('[GATE 6] [UAT-04] STEP 10/11: Opening Persona Context Menu');
    ...
    await test.step('11. Execute Sign Out / Logout & Verify Cache Purge', async () => {
      console.log('[GATE 6] [UAT-04] STEP 11/11: Executing Complete Logout');
    ...
    console.log('============================================================');
    console.log('[GATE 6] [UAT-04] COMPLETED SUCCESSFULLY: ALL STEPS PASS');
    console.log('============================================================');
  });
});
