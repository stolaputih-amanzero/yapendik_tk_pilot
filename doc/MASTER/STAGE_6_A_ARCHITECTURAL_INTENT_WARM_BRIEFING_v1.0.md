...
### Design Brief & Architectural Intent Document (v1.0-DRAFT)

**META**

| Atribut | Nilai |
| --- | --- |
| Document ID | `DOC-AMANAURA-STAGE-6A-BRIEFING-INTENT-v1.0` |
| Version | `1.0-DRAFT` (Architectural Intent) |
...
| Target Scope | Context-Aware Briefing System untuk 4 Peran (Guru, KS, Yayasan, Guardian) |
...
### 1.3 The Solution: 4 Briefing Machines, 1 Shell
...
Empat mesin, satu `<BriefingShell>`. Dan tak satu pun dari mereka dashboard: masing-masing **satu kalimat + satu aksi + satu gema**.
...
**Keputusan:** Override per kelas ditangguhkan ke v2 bila ada bukti kebutuhan nyata. v1 hanya mendukung konfigurasi tingkat sekolah oleh Kepala Sekolah.

### 2.2 Mechanism: Briefing as Workspace Header, Not Separate Page

**Bukan halaman gerbang terpisah** — briefing menggantikan header generik di workspace masing-masing.

**Struktur:**
```tsx
<BriefingShell>           // Shared: sapaan waktu, tanggal, avatar
  <TeacherBriefing />     // Khusus guru: fase aktif, pending, CTA
  <HeadmasterBriefing />  // Khusus KS: adopsi, rekonsiliasi, alert
  <FoundationBriefing />  // Khusus yayasan: insight, action ledger
  <GuardianBriefing />    // Khusus ortu: kabar anak hari ini
</BriefingShell>
```
...
* **Mode "hari usai"** — di luar jam sekolah, briefing berubah menjadi refleksi penutup atau pratinjau esok.
* **Fail-safe defaulting** — konfigurasi hilang → jatuh ke bawaan kanonikal.
...
| **Service layer** | Tidak berubah | Baru: `BriefingEngine` (komposisi per peran) |
| **Komponen UI** | Refactor existing | Baru: `BriefingShell` + 4 anak komponen |
| **Protokol** | UI/UX-Only Protocol (§9.3) | **Stage Gate Process** (Gate 0 → 0.1 → 1) |

### 4.2 Proposed New Invarian

**FB-08 (baru): School Rhythm Autonomy**
...
**FB-09 (baru): Guardian Data Minimization**
...
**H-07 (baru): Briefing Non-Surveillance**
...
export interface PhaseConfig {
  phase_id: string; // Kanonikal: 'WELCOME', 'CENTRA', 'LUNCH', etc.
  phase_name: string;
...
export interface PhaseActionMapping {
  action_id: string;
  action_name: string; // "Rekam Momen", "Buka Presensi", etc.
  action_type: 'NAVIGATION' | 'MODAL' | 'SHEET';
  target_route?: string;
  target_component?: string;
  is_default: boolean; // Bawaan kanonikal
}

// 3. BRIEFING DATA (Komposisi per peran)
export interface TeacherBriefingData {
  greeting: string; // "Selamat pagi, Bu Siti"
  date: string;
  ...
  warm_echo?: string; // Kutipan dari ortu atau refleksi guru
}

export interface HeadmasterBriefingData {
  greeting: string;
  date: string;
  reconciliation: {
...
export interface FoundationBriefingData {
  greeting: string;
  date: string;
  decision_queue: {
  ...
  warm_echo?: string; // human_reflective_interpretation dari KS
}

export interface GuardianBriefingData {
  greeting: string;
  date: string;
  child_name: string;
  today_summary: {
...
* **FB-03 (Autonomous Unit Leadership):** Konfigurasi ritme adalah hak KS; Yayasan tidak boleh override.
...
* Mengunci entitas domain (SchoolRhythmConfig, PhaseActionMapping, BriefingData).
* Mengunci state machines (bukan state machine baru, tapi query composition rules).
* Mengunci API contracts (getBriefingData per role).
...
* Mengunci semantik non-surveilans (H-07: Briefing Non-Surveillance).

### Gate 1: Technical Architecture & Enforcement Design
* Skema database baru + RLS policies.
* Service layer (BriefingEngine).
* Adversarial test suites (FB-08, FB-09, H-07).
* Komponen UI (BriefingShell + 4 anak komponen).

---

## 7. NEXT STEPS

Dokumen ini adalah **design brief** yang siap untuk review. Ketika Project Owner memutuskan untuk melanjutkan:

1. **Review dokumen ini** dan berikan feedback atau veto.
2. **Inisiasi Gate 0** untuk mengunci kontrak semantik.
3. **Lanjutkan ke Gate 0.1 dan Gate 1** sesuai proses standar.
4. **Implementasi** dengan Stage 4.5 FROZEN dan UI/UX-Only Protocol untuk lapisan presentasi.
...
**Status:** Dokumen ini adalah **Architectural Intent** yang siap untuk review. Tidak ada kode yang akan ditulis sampai Project Owner memberikan persetujuan untuk memulai Gate 0.
