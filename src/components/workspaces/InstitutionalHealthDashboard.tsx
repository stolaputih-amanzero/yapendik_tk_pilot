  ...
  return (
    <div className="max-w-7xl mx-auto p-6 space-y-6 text-slate-100 font-sans">
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 relative z-10">
          <div>
            <div className="flex items-center space-x-2 text-emerald-400 text-xs font-bold tracking-wider uppercase mb-1">
              ...
              <span>Stage 3.4 • Institutional Health Telemetry & Intelligence</span>
            </div>
            <h1 className="text-2xl font-bold text-white tracking-tight">
              Monitor Kesehatan Kelembagaan & Telemetri
            </h1>
            <p className="text-slate-400 text-sm mt-1">
              {activeSchool?.name || 'TK Yapendik'} • 
              {telemetry?.academic_year_name ? ` ${telemetry.academic_year_name} (${telemetry.semester}) • ` : ' '}
              Kalkulasi kecerdasan operasional turunan langsung (*Zero Stale Tables*).
            </p>
          </div>
          ...
          <div className="flex flex-wrap items-center gap-3">
            ...
            {isSuperadmin && schools.length > 1 && (
              <select
                ...
                className="bg-slate-950 border border-slate-800 rounded-xl px-3 py-2 text-xs text-slate-200 font-semibold focus:outline-none focus:border-emerald-500"
              >
                {schools.map(s => (
            ...
            {/* Health Status Pill */}
            <div className={`px-3.5 py-1.5 rounded-full border text-xs font-bold flex items-center space-x-2 ${currentBadge.bg}`}>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        ...
        <div className="bg-slate-900 border border-slate-800 rounded-2xl p-5 shadow-lg flex flex-col justify-between space-y-4 hover:border-slate-700 transition-all">
          <div>
      ...
      <div className="bg-slate-900 border border-slate-800 rounded-2xl p-6 shadow-xl space-y-4">
        <div className="flex items-center justify-between border-b border-slate-800 pb-3">
          <div className="flex items-center space-x-2">
            ...
            <h3 className="text-base font-bold text-white">Daftar Eksepsi & Diagnostik Operasional Real-Time</h3>
          </div>
        ...
        {exceptions.length > 0 ? (
          <div className="space-y-2.5">
            {exceptions.map((ex, idx) => (
              <div 
                key={idx} 
                className="p-3.5 bg-slate-950/80 border border-slate-800 rounded-xl flex items-start space-x-3 text-xs"
              >
                <AlertTriangle className="w-4 h-4 flex-shrink-0 text-amber-400 mt-0.5" />
                <div className="flex-1">
                  <div className="flex items-center space-x-2">
                    <span className="font-mono font-bold text-amber-300">{ex.code}</span>
                    <span className="text-[10px] bg-slate-800 text-slate-400 px-1.5 py-0.5 rounded font-mono">SYSTEM EXCEPTION</span>
                  </div>
          ...
          <div className="p-6 bg-slate-950/40 border border-slate-800/80 rounded-xl flex items-center space-x-3 text-xs text-emerald-400">
            <CheckCircle2 className="w-5 h-5 flex-shrink-0" />
            <div>
              <p className="font-bold">Seluruh Parameter Operasional Berjalan Normal</p>
              <p className="text-slate-400 mt-0.5">Tidak ada eksepsi kelembagaan atau pelanggaran kapasitas yang terdeteksi saat ini.</p>
            </div>
          </div>
        )}
      </div>
        ...
        <div className="bg-slate-900 border border-slate-800 rounded-2xl p-6 shadow-xl space-y-4">
          <div className="flex items-center justify-between border-b border-slate-800 pb-3">
            <div className="flex items-center space-x-2">
              ...
              <h3 className="text-base font-bold text-white">Matriks Kesehatan Multi-Unit Sekolah (Yayasan)</h3>
            </div>
            <span className="text-xs text-slate-400 font-semibold">{multiSchoolData.length} Unit Sekolah Terpantau</span>
          </div>
                ...
                <div 
                  ...
                  className={`p-4 rounded-xl border transition-all cursor-pointer ${
                    item.schoolId === currentSchoolId 
                      ? 'bg-slate-800/80 border-emerald-500/50 shadow-lg' 
                      : 'bg-slate-950/60 border-slate-800 hover:border-slate-700'
                  }`}
                >
                  <div className="flex items-center justify-between mb-2">
                    <span className="font-bold text-white text-xs">{item.schoolName}</span>
                    <span className={`px-2 py-0.5 rounded text-[10px] font-bold uppercase ${b.bg}`}>
                      {itemStatus}
                    </span>
                  </div>

                  <div className="grid grid-cols-2 gap-2 text-[11px] text-slate-400 mt-3 pt-3 border-t border-slate-800">
                    <div>
                      <span>Utilisasi:</span>
