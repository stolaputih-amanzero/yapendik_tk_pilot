...
export const InstitutionalHealthDashboard: React.FC = () => {
  ...
  return (
    <div className="w-full max-w-7xl mx-auto px-4 md:px-6 py-6 space-y-6 text-slate-900 font-sans">
      {/* Header Banner */}
      <div className="bg-slate-50 border-y md:border border-slate-200 p-4 md:p-6 relative overflow-hidden md:rounded-2xl md:shadow-sm -mx-4 md:mx-0">
          ...
          <div className="flex flex-col md:flex-row items-stretch md:items-center gap-3 mt-4 md:mt-0 w-full md:w-auto">
            ...
            {isSuperadmin && schools.length > 1 && (
              <select
                ...
                className="w-full flex justify-between items-center bg-white border border-slate-300 rounded-xl px-3 py-2 text-xs text-slate-800 font-semibold focus:outline-none focus:border-emerald-500"
              >
                {schools.map(s => (
            ...
            <button
              ...
              disabled={refreshing}
              className="w-full md:w-auto px-3.5 py-2.5 md:py-2 rounded-xl bg-white hover:bg-slate-50 text-slate-700 border border-slate-200 text-xs font-semibold flex justify-center items-center space-x-2 transition-all shadow-xs"
            >
