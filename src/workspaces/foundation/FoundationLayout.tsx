...
import { PrivacyShield } from '../../components/glass/PrivacyShield';
import { 
  Building2, 
  ShieldCheck, 
  BarChart3, 
...
import { InitiativeCreatorModal } from '../../components/workspaces/foundation/InitiativeCreatorModal';
...
export const FoundationLayout: React.FC<FoundationLayoutProps> = ({ 
  ...
  const handleInitiativeCreated = async (newAction: InstitutionalActionRecord) => {
    await refreshData();
  ...
  return (
    <ForbiddenActionGate>
      ...
      >
        {/* ─────────────────────────────────────────────────────────────
            ZONA 1: SIRKADIAN BRIEFING EKSEKUTIF (Hero Banner)
            ───────────────────────────────────────────────────────────── */}
        ...
        <section className="py-8 space-y-4">
          <div className="flex items-center justify-between">
            <div className="space-y-1">
              ...
              <h2 className="text-xl medium:text-2xl font-bold text-ink leading-tight tracking-tight flex items-center gap-2">
                ...
                <span>Pusat Kemudi Strategis</span>
              </h2>
              <p className="text-xs text-ink-soft">
                Pintasan instrumen pengawasan mutu, penerbitan inisiatif, kesiapan akreditasi unit, dan registrasi PPDB.
              </p>
            </div>
          </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3.5">
            ...
            >
              ...
              <h3 className="font-bold text-sm text-ink group-hover:text-brand-primary transition-colors">
                Terbitkan Inisiatif
              </h3>
              <p className="text-xs text-ink-soft mt-1 leading-snug">
                Salurkan bantuan APE, pelatihan guru, atau arahan kebijakan ke unit.
              </p>
            </button>

            {/* Card 2: Audit Tata Kelola */}
            <button
              type="button"
            ...
            >
              ...
              <p className="text-xs text-ink-soft mt-1 leading-snug">
                Inspeksi jejak audit kelembagaan dan log keamanan seluruh unit.
              </p>
            </button>

            {/* Card 3: Kesiapan Unit */}
            <button
              type="button"
            ...
            >
              ...
              <p className="text-xs text-ink-soft mt-1 leading-snug">
                Matriks registrasi sekolah, rombel, dan checklist akreditasi.
              </p>
            </button>

            {/* Card 4: Portal PPDB Global */}
            <button
              type="button"
            ...
            >
              ...
              <p className="text-xs text-ink-soft mt-1 leading-snug">
                Pantau daya tampung dan alur registrasi penerimaan murid baru.
              </p>
            </button>
          </div>
        </section>
        ...
        {/* ─────────────────────────────────────────────────────────────
            ZONA 3: TELEMETRI MAKRO JARINGAN SEKOLAH (Network Vitality Telemetry)
            ───────────────────────────────────────────────────────────── */}
        <section className="py-8 space-y-4">
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2">
            <div className="space-y-1">
              <span className="text-[11px] font-bold uppercase tracking-wider text-brand-deep block">
                Telemetri Kurikulum Merdeka PAUD (Zero-PII)
              </span>
              <h2 className="text-xl medium:text-2xl font-bold text-ink leading-tight tracking-tight flex items-center gap-2">
                <BarChart3 className="w-6 h-6 text-brand-primary shrink-0" />
                <span>Denyut Capaian Jaringan Sekolah</span>
              </h2>
            </div>
            <div className="flex items-center gap-2">
              <span className="px-3 py-1 rounded-full text-xs font-mono font-bold bg-success-tint text-success-deep border border-success-line">
                K-Anonymity Aktif (N &ge; 5)
              </span>
            </div>
          </div>

          {/* Privacy Guarantee Note (R-4) */}
          ...
          {/* Domain Telemetry Cards */}
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 pt-2">
            {patterns.map((pat) => {
              const isSuppressed = pat.exposure_status !== 'VISIBLE';

              return (
                <div 
                  key={pat.pattern_id}
                  className="p-4 rounded-2xl bg-surface border border-line space-y-3 shadow-hairline flex flex-col justify-between"
                  data-testid="pattern-telemetry-card"
                >
                  <div className="space-y-1.5">
                    <div className="flex items-center justify-between">
                      <span className="text-[10px] font-mono uppercase tracking-wider px-2 py-0.5 rounded-full bg-surface-subtle text-ink-soft border border-line-hairline font-bold">
                        {pat.curriculum_domain || 'DOMAIN'}
                      </span>
                      <span className="text-[10px] font-mono font-bold text-success-deep bg-success-tint px-2 py-0.5 rounded-full border border-success-line">
                  ...
                  <div className="pt-2 border-t border-line-hairline flex items-center justify-between">
                    <span className="text-[11px] text-ink-faint font-mono">
                      Sampel: {pat.cohort_size} Siswa
                    </span>
                    {isSuppressed ? (
                      <PrivacyShield 
                        exposureStatus={pat.exposure_status}
                        sampleSize={pat.cohort_size}
                      />
                    ) : (
                      <span className="text-lg font-bold font-mono text-ink">
                        {pat.computed_metric_value ? `${pat.computed_metric_value.toFixed(1)}%` : '—'}
                      </span>
                    )}
                  </div>
                </div>
        ...
        {/* ─────────────────────────────────────────────────────────────
            ZONA 4: BUKU BESAR TATA KELOLA CLOSED-LOOP (FB-05 / FB-03)
            ───────────────────────────────────────────────────────────── */}
              ...
              <div className="flex items-center gap-2 shrink-0">
                ...
                <div className="bg-surface-subtle border border-line-hairline px-3 py-1 rounded-xl text-right">
                  ...
                  <div className="text-xs font-bold text-ink">
                    {currentPersona?.name || 'SHIRLEY A.T.WAKKARY'}
                  </div>
                </div>
              </div>
            </div>

            {/* Navigation Pills Flat (R-2 Kontrol Flat) */}
